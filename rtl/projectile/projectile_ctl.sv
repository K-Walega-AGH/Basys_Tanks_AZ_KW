...
(
    ...
    logic collision_enemy, collision_terrain;

    // signals assignments
    assign enemy_hit = (projectile_st == COLLISION_ENEMY);
    assign terrain_hit = (projectile_st == COLLISION_TERRAIN);
    assign activate_explosion = ((projectile_st == COLLISION_ENEMY) || (projectile_st == COLLISION_TERRAIN));
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // case dedicated to calculating bullet velocities
            case (projectile_st)
                IDLE: begin
                    show_bullet <= '0;
                    vx          <= '0;
            ...
            endcase
            // case for collision detection (and stopping bullet)
            case(projectile_st)
                FALLING, RISING: begin
                    // check for collision with enemy tank
                    if ( (projectile_xpos+PROJECTILE_RADIUS/2 >= enemy_xpos && projectile_xpos+PROJECTILE_RADIUS/2 < enemy_xpos + TANK_WIDTH) &&
                         (projectile_ypos+PROJECTILE_RADIUS/2 >= enemy_ypos + TANK_EMPTY_HEIGHT && projectile_ypos+PROJECTILE_RADIUS/2 < enemy_ypos + TANK_HEIGHT) ) begin
                        collision_enemy <= 1;
                        vx <= '0;
                        vy <= '0;
                        temp_vx     <= '0;
                    ...
                    //
                    if(hcount == (projectile_xpos+PROJECTILE_RADIUS/2)) begin
                        // check for collision with terrain
                        if(terrain_y <= (projectile_ypos+PROJECTILE_RADIUS)) begin
                            vx <= '0;
                            vy <= '0;
                            temp_vx     <= '0;
                            temp_vy     <= '0;
                ...
                COLLISION_ENEMY: begin
                    collision_enemy <= 0;
                    show_bullet <= '0;
                    explosion_xpos <= projectile_xpos;
                    explosion_ypos <= projectile_ypos;
                end
                COLLISION_TERRAIN: begin
                    collision_terrain <= 0;
                    show_bullet <= '0;
                    explosion_xpos <= projectile_xpos;
                    explosion_ypos <= projectile_ypos;
    ...
    always_comb begin
        ...
        case (projectile_st)
            ...
            RISING: begin
                if (collision_enemy)
                    projectile_st_nxt = COLLISION_ENEMY;
                else if (collision_terrain)
                    projectile_st_nxt = COLLISION_TERRAIN;
                else if(vy <= GRAVITY)
            ...
            FALLING: begin
                if (collision_enemy)
                    ...
                    projectile_st_nxt = COLLISION_TERRAIN;
                else if( (xpos_nxt[31:20] - vx[31:20] <= 0 || xpos_nxt[31:20] + vx[31:20] > HOR_PIXELS) ||
