module ps2_keyboard #(parameter PLAYER_NUMBER = 1) (
    input  logic clk100MHz,
    input  logic clk60MHz,
    input  logic rst,

    input  logic       player_turn,
    input  logic       P1andP2_ready,
    input  logic       OneBasysMode,
    ...
    //         rx_data   = (player_turn || OneBasysMode) ? ps2_rx_data : uart_rx_data;
    //         read_data = (player_turn || OneBasysMode) ? ps2_read_data : uart_read_data;
    //     end else begin
    //         rx_data   = (!player_turn || OneBasysMode) ? ps2_rx_data : uart_rx_data;
    //         read_data = (!player_turn || OneBasysMode) ? ps2_read_data : uart_read_data;
    //     end
    // end
     // simply put - if sw[2] on then always ps2, else based on player's bitstream choose uart or ps2

    assign rx_data   = (PLAYER_NUMBER == 1)
                   ? ((player_turn || OneBasysMode) ? ps2_rx_data : uart_rx_data)
                   : ((!P1andP2_ready || !player_turn || OneBasysMode) ? ps2_rx_data : uart_rx_data);

    assign read_data = (PLAYER_NUMBER == 1)
                   ? ((player_turn || OneBasysMode) ? ps2_read_data : uart_read_data)
                   : ((!P1andP2_ready || !player_turn || OneBasysMode) ? ps2_read_data : uart_read_data);
